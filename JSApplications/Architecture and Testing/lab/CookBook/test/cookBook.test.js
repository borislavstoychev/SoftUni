...
describe('E2E tests', function () {
    ...
    describe('Home', () => {
        it('show most recent recipes', async () => {
            page.route('**' + endpoints.create + "*", route => route.fulfill(json(mockData.list)));

            await page.goto(host);

            const titles = await page.$$eval('article.preview .title', t => t.map(s => s.textContent));
    ...
    describe('CRUD', () => {
        ...
        it('author sees edit and delete buttons', async () => {
            page.route('**' + endpoints.create + '*', route => route.fulfill(json([mock])));
            const mock = {
                name: 'Name1',
                img: '/assests/new.png',
                ingredients: ['i1', 'i2'],
